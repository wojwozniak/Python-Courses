...
def create_spend_chart(categories):
  ...
  for category in categories:
    category_expense = sum([-transaction['amount'] for transaction in category.ledger if transaction['amount'] < 0])
  ...
  category_expenses = [(expense / total_expenses) * 100 for expense in category_expenses]
  category_labels = [label.ljust(max_label_length, " ") for label in category_labels]
